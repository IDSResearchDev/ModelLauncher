...
namespace Rnd.Common
{
    public class Utilities
    {
        ...
        /// <summary>
        /// Helper method to create a folder anywhere if path is specified
        /// </summary>
        /// <param name="path">the location of the folder</param>
        public void CreateFolder(string path)
        {
            var dir = path + "/ModelLauncher";
        ...
        /// <returns></returns>
        public static string PathFilename(string path, string filename, string extension)
        {
            return (extension[0] == '.')
        ...
        /// <returns></returns>
        public static bool CheckIfDirectoryExists(string dir)
        {
        ...
        /// <returns></returns>
        public static bool CheckIfFileExists(string pathfilename)
        {
        ...
        /// <param name="appendvalue">Value to write.</param>
        /// <param name="existingvalue">Check and append existing line value.</param>
        public static void CustomReadWriteLines(string filename, string appendvalue, string existingvalue)
        {
            var tempfile = Path.GetTempFileName();
            var counter = 0;
            using (var writer = new StreamWriter(tempfile))
        ...
        {
            var temp = ReadAllLines(Path.Combine(modelPath, ".This_is_multiuser_model"));
        ...
        {
            using (StreamWriter sw = File.CreateText(filename))//new StreamWriter(filename, true))//
            {
                sw.Write(text);
            }  
        }
        ...
        {
            var filename = Path.Combine(modelPath, ".This_is_multiuser_model");
        ...
        /// <summary>
        /// Copy specific file to another directory
        /// </summary>
        /// <param name="source">Directory of the files to copy</param>
        /// <param name="destination"></param>
        public void CopyFilesToLocation(string source, string destination, string filename, string extension)
        {
            if (Directory.Exists(destination))
            {
                var util = new Common.Utilities();
                var items = util.GetFiles(source, filename, extension);
        ...
        /// <summary>
        /// Get single file on a specified path and extension
        /// </summary>
        /// <param name="path"></param>
        /// <param name="extension"></param>
        /// <returns></returns>
        public FileInfo[] GetFiles(string path, string filename, string extension)
        {
            DirectoryInfo dir = new DirectoryInfo(path);
            FileInfo[] files = dir.GetFiles(filename + "." + extension);
            return files;
        }
